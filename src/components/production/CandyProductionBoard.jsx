...
const COLUMNS = [
  ...
  { key: 'complete',      label: 'Complete',     statuses: ['complete', 'packaged', 'released'] },
]
const COLUMN_STATUS = { backlog: 'backlog', queued: 'queued', in_production: 'in_production', complete: 'complete' }
const STATUS_NEXT = { backlog: 'queued', scheduled: 'queued', hold: 'queued', queued: 'in_production', in_production: 'complete' }
const COL_COLOR = { backlog: '#6b7280', queued: '#d97706', in_production: '#1d4ed8', complete: '#16a34a' }

function colForStatus(status) {
  return COLUMNS.find(c => c.statuses.includes(status))
}

export default function CandyProductionBoard() {
  const [batches, setBatches] = useState([])
  const [loading, setLoading] = useState(true)
  const [advancing, setAdvancing] = useState(null)
  const [activeDragId, setActiveDragId] = useState(null)
  const [pendingMove, setPendingMove] = useState(null)     // → StartProductionModal
  const [pendingComplete, setPendingComplete] = useState(null) // → CompleteProductionModal
  ...
  async function advanceBatch(batch) {
    ...
    if (next === 'in_production') { setPendingMove({ batch }); return }
    if (next === 'complete') { setPendingComplete(batch); return }
  ...
  function handleDragEnd({ active, over }) {
    ...
    if (srcCol.key === dstColKey) {
      ...
      setPendingComplete(activeBatch)
    } else {
  ...
  function handleCompleteConfirmed(updatedBatch) {
    setBatches(prev => prev.map(b => b.id === updatedBatch.id ? updatedBatch : b))
  ...
  return (
    <div>
      <div style={s.toolbar}>
        <button style={s.newBtn} onClick={() => setShowNewBatch(true)}>+ New Work Order</button>
      </div>
          ...
          <div style={s.board}>
            {COLUMNS.map(col => (
              <KanbanColumn
                ...
                advancing={advancing} onAdvance={advanceBatch} onCardClick={setSelectedBatch}
              />
            ))}
          </div>
          <DragOverlay>
      ...
      {pendingComplete && (
        <Modal title={`Complete: ${pendingComplete.productName}`} onClose={() => setPendingComplete(null)} maxWidth={420}>
          <CompleteProductionModal
            batch={pendingComplete}
      ...
      {showNewBatch && (
        <Modal title="New Work Order" onClose={() => setShowNewBatch(false)} maxWidth={560}>
...
function BatchCard({ batch, canAdvance, advancing, onAdvance, onCardClick }) {
  ...
  return (
    <div style={s.card} onClick={() => onCardClick(batch)}>
      ...
      {isHold && <span style={s.holdBadge}>Hold</span>}
      {canAdvance && !isHold && (
...
const s = {
  ...
  newBtn: { background: '#1d4ed8', color: '#fff', border: 'none', borderRadius: 8, padding: '0.5rem 1rem', fontSize: '0.875rem', fontWeight: 600, cursor: 'pointer' },
