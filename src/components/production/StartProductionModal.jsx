...
export default function StartProductionModal({ batch, onClose, onConfirmed }) {
  ...
  async function handleIngredientChange(idx, ingredientId) {
    setRows(prev => prev.map((r, i) => i === idx ? { ...r, ingredientId, ingredientName: '', lotId: '', lots: [], quantityUsed: '' } : r))
    ...
    try {
      ...
      await runTransaction(db, async (tx) => {
        ...
        for (const row of validRows) {
          ...
          tx.set(doc(collection(db, 'ingredientUsage')), {
        ...
        tx.update(doc(db, 'productionBatches', batch.id), {
          ...
          auditLog: (batch.auditLog || []).concat([{ action: 'moved to in_production', changedBy: userInfo, changedAt: Timestamp.now() }]),
        })
      })

      onConfirmed({ ...batch, status: 'in_production', quantityProduced: qty, unit })
      onClose()
    } catch (err) {
      alert(err.message)
    }
    ...
    <Modal title={`Start Production — ${batch.batchNumber}`} onClose={onClose} maxWidth={540}>
      <div>
        ...
        <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '0.875rem' }}>
          <input style={{ ...input, marginBottom: 0, flex: 2 }} type="number" min="0" step="any" value={batchSize} onChange={e => setBatchSize(e.target.value)} placeholder="0" autoFocus />
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
          ...
          <button type="button" style={{ background: 'none', border: 'none', color: '#1d4ed8', fontSize: '0.8rem', fontWeight: 600, cursor: 'pointer' }}
            onClick={() => setRows(prev => [...prev, { ingredientId: '', ingredientName: '', lotId: '', lots: [], quantityUsed: '' }])}>
            + Add ingredient
          </button>
        </div>

        {rows.length === 0 && (
          <p style={{ fontSize: '0.8rem', color: '#9ca3af', marginBottom: '0.875rem' }}>No ingredients added — tap above to add.</p>
        )}
        ...
        {rows.map((row, idx) => (
          <div key={idx} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 80px 28px', gap: '0.375rem', marginBottom: '0.5rem', alignItems: 'center' }}>
            ...
            <button type="button" onClick={() => setRows(prev => prev.filter((_, i) => i !== idx))} style={{ background: 'none', border: 'none', color: '#9ca3af', fontSize: '1rem', cursor: 'pointer', padding: 0 }}>✕</button>
          </div>
        ))}
