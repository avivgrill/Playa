...
const STATUS_LABELS = {
  scheduled: 'Scheduled', in_production: 'In Production',
  complete: 'Complete', hold: 'Hold', released: 'Released',
}
const STATUSES = ['scheduled', 'in_production', 'complete', 'hold', 'released']
...
export default function BatchDetail() {
  ...
  async function load() {
    const [batchSnap, logsSnap, allocSnap, usageSnap] = await Promise.all([
      ...
      getDocs(query(collection(db, 'productionLogs'), where('batchId', '==', id), orderBy('startTime', 'desc'))),
      getDocs(query(collection(db, 'batchAllocations'), where('batchId', '==', id), orderBy('allocationDate', 'desc'))),
  ...
  useEffect(() => {
    load()
    getDocs(query(collection(db, 'customers'), where('status', '==', 'active'), orderBy('customerName', 'asc')))
  ...
  if (!batch) return <p style={{ color: '#dc2626', padding: '1rem' }}>Batch not found.</p>
  ...
  return (
    <div>
      <button style={backBtn} onClick={() => navigate('/operations/batches')}>← Production Batches</button>
      ...
      <div style={{ display: 'flex', gap: 0, borderBottom: '2px solid #e5e7eb', marginBottom: '1rem', overflowX: 'auto' }}>
        {[
          ['overview', 'Overview'],
          ['ingredients', `Ingredients (${usageRecords.length})`],
          ['logs', `Logs (${logs.length})`],
          ['allocations', `Allocations (${allocations.length})`],
        ].map(([key, lbl]) => (
          <button key={key}
      ...
      {tab === 'overview' && (
        <div>
          <div style={card}>
            <Row label="Production Date" value={formatDate(batch.productionDate)} />
            {batch.sopName && <Row label="SOP" value={batch.sopName} />}
            <Row label="Quantity Produced" value={batch.quantityProduced ? `${batch.quantityProduced.toLocaleString()} ${batch.unit}` : '—'} />
            <Row label="Quantity Allocated" value={totalAllocated > 0 ? `${totalAllocated.toLocaleString()} ${batch.unit}` : '—'} />
            <Row label="Available" value={`${available.toLocaleString()} ${batch.unit}`} />
            <Row label="Created by" value={batch.createdBy?.displayName || batch.createdBy?.email} />
          </div>
          {batch.notes && (
            <div style={card}>
        ...
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
            <p style={{ fontSize: '0.875rem', color: '#6b7280', margin: 0 }}>
              Select ingredient → system suggests oldest lot first
            </p>
            <button style={btn.primary} onClick={() => { setShowUsageForm(!showUsageForm); setUsageForm({ ingredientId: '', lotId: '', quantityUsed: '' }); setAvailableLots([]) }}>
              {showUsageForm ? 'Cancel' : '+ Add Ingredient'}
            </button>
          </div>

          {showUsageForm && (
            <form onSubmit={submitUsage} style={{ ...card, background: '#f0f9ff', borderLeft: '4px solid #1d4ed8', marginBottom: '1rem' }}>
              <h3 style={{ fontSize: '0.9rem', fontWeight: 700, color: '#1d4ed8', marginBottom: '0.75rem' }}>Add Ingredient</h3>
              ...
              {loadingLots && <p style={{ color: '#9ca3af', fontSize: '0.85rem', marginBottom: '0.875rem' }}>Loading lots…</p>}
              ...
              {!loadingLots && usageForm.ingredientId && availableLots.length === 0 && (
                <div style={{ background: '#fef2f2', borderRadius: 8, padding: '0.6rem 0.875rem', fontSize: '0.85rem', color: '#dc2626', marginBottom: '0.875rem' }}>
                  No available stock for this ingredient.
                </div>
              )}

              {availableLots.length > 0 && (
                <>
                  <label style={label}>Lot <span style={{ color: '#16a34a', fontSize: '0.8rem' }}>(oldest first — FIFO)</span></label>
                  ...
                  {selectedLot && (
                    <div style={{ background: '#f0fdf4', borderRadius: 8, padding: '0.5rem 0.875rem', fontSize: '0.85rem', color: '#15803d', marginBottom: '0.875rem' }}>
                      Available: <strong>{selectedLot.currentQuantity.toLocaleString()} {selectedLot.unit}</strong>
                      {selectedLot.supplierLotNumber ? ` · Supplier lot: ${selectedLot.supplierLotNumber}` : ''}
                    </div>
                  )}

                  <label style={label}>Quantity Used ({selectedLot?.unit || ''})</label>
                  ...
                  <button type="submit" style={btn.primary} disabled={addingUsage}>
                    {addingUsage ? 'Adding…' : 'Add to Batch'}
                  </button>
                </>
              )}
            </form>
          ...
          {usageRecords.length === 0 && !showUsageForm && (
            <p style={muted}>No ingredients added yet. Tap "+ Add Ingredient" to start.</p>
          )}
            ...
            <div key={u.id} style={{ ...card, cursor: 'pointer' }} onClick={() => navigate(`/operations/lots/${u.lotId}`)}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '0.5rem' }}>
                <div>
                  ...
                  {u.supplierLotNumber && <div style={{ fontSize: '0.75rem', color: '#9ca3af' }}>Supplier: {u.supplierLotNumber}</div>}
      ...
      {tab === 'logs' && (
        <div>
          ...
          {logs.length === 0 && <p style={muted}>No production logs yet.</p>}
          {logs.map(log => (
        ...
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
            <div style={{ fontSize: '0.875rem', color: '#6b7280' }}>
              Available: <strong>{available.toLocaleString()} {batch.unit}</strong>
            </div>
          ...
          {showAllocForm && (
            <form onSubmit={submitAllocation} style={{ ...card, background: '#f0f9ff', borderLeft: '4px solid #1d4ed8' }}>
              <h3 style={{ fontSize: '0.9rem', fontWeight: 700, color: '#1d4ed8', marginBottom: '0.75rem' }}>New Allocation</h3>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
                <div>
                  <label style={label}>Quantity ({batch.unit}) *</label>
              ...
              <input style={input} value={allocForm.notes} onChange={e => setAllocForm(prev => ({ ...prev, notes: e.target.value }))} placeholder="Optional notes…" />
              <button type="submit" style={btn.primary} disabled={allocating}>{allocating ? 'Saving…' : 'Save Allocation'}</button>
            </form>
          )}

          {allocations.length === 0 && !showAllocForm && <p style={muted}>No allocations yet.</p>}
          {allocations.map(a => (
...
function statusColor(s) {
  return { scheduled: '#d97706', in_production: '#1d4ed8', complete: '#7e22ce', hold: '#dc2626', released: '#16a34a' }[s] || '#6b7280'
}
