...
import NewCardModal from '../components/production/NewCardModal'
import StartProductionModal from '../components/production/StartProductionModal'
import CardDetailSlideOver from '../components/production/CardDetailSlideOver'
...
export default function ProductionLanding() {
  ...
  const [pendingMove, setPendingMove] = useState(null) // { batch, dstColKey }
  ...
  function handleDragEnd({ active, over }) {
    ...
    if (srcCol.key === dstColKey) {
      ...
      setPendingMove({ batch: activeBatch, dstColKey })
    } else {
      moveToColumn(activeBatch, dstColKey, over.id)
    }
  }

  function handleCardCreated(newBatch) {
  ...
  function handleBatchDeleted(batchId) {
  ...
  return (
    <div>
      <div style={s.header}>
        <div>
          <h1 style={s.title}>Production</h1>
          <div style={s.quickLinks}>
            <Link to="/operations/client-orders" style={s.qLink}>Client Orders</Link>
      ...
      {loading ? (
        ...
        <DndContext sensors={sensors} collisionDetection={closestCorners} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
          <div style={s.board}>
            {COLUMNS.map(col => (
              <KanbanColumn key={col.key} col={col} batches={colBatches(col)} onCardClick={setDetailBatch} />
            ))}
          </div>
          <DragOverlay>
...
function KanbanColumn({ col, batches, onCardClick }) {
    ...
    <div style={s.column}>
      ...
      <SortableContext items={batches.map(b => b.id)} strategy={verticalListSortingStrategy}>
        <div ref={setNodeRef} style={s.colBody}>
          {batches.map(batch => (
            <SortableCard key={batch.id} batch={batch} onCardClick={onCardClick} />
          ))}
...
function SortableCard({ batch, onCardClick }) {
  ...
  return (
    <div ref={setNodeRef} style={{ transform: CSS.Transform.toString(transform), transition, opacity: isDragging ? 0.3 : 1 }} {...attributes} {...listeners}>
      <BatchCard batch={batch} onClick={() => onCardClick(batch)} />
    </div>
  )
}
...
function BatchCard({ batch, onClick }) {
  ...
  return (
    <div style={s.card} onClick={onClick}>
      ...
      {batch.sopName && <div style={s.meta}>{batch.sopName}</div>}
      {batch.actualQuantity > 0 && <div style={s.meta}>{batch.actualQuantity.toLocaleString()} {batch.unit}</div>}
...
const s = {
  ...
  productName: { fontSize: '0.85rem', fontWeight: 600, color: '#111827', lineHeight: 1.3, marginBottom: '0.2rem' },
