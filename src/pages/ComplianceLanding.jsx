...
export default function ComplianceLanding() {
  ...
  // activeModal: null | { mode: 'view', type, id } | { mode: 'start', type } | 'cleaning'
  ...
  function handleInspectionClick(type, done, id) {
    if (done && id) {
      setActiveModal({ mode: 'view', type, id })
    } else {
      setActiveModal({ mode: 'start', type })
    }
  }
  ...
  const modalTitle = activeModal === 'cleaning'
    ? t('New Cleaning Log')
    : activeModal?.type
      ? INSPECTION_TITLES[activeModal.type]
      : ''
  ...
  return (
    <div>
      ...
      <Section label={t('Actions')}>
        <ActionRow icon="🧹" label={t('New Cleaning Log')} onClick={() => setActiveModal('cleaning')} />
        <ActionRow
          icon="⚠️"
          label={
      ...
      {/* Center modals */}
      {activeModal && (
        <Modal title={modalTitle} onClose={closeModal} maxWidth={activeModal === 'cleaning' ? 480 : 640}>
          {activeModal === 'cleaning' && (
            <NewCleaningLog onClose={closeModal} />
          )}
          {activeModal?.mode === 'view' && (
            <InspectionDetail id={activeModal.id} onClose={closeModal} />
          )}
          {activeModal?.mode === 'start' && (
            <StartInspection type={activeModal.type} onClose={closeModal} />
          )}
        </Modal>
      )}
    </div>
...
function InspectionRow({ label, done, loading, onClick }) {
  ...
  return (
    <button onClick={onClick} style={row} disabled={loading}>
      <span style={{ fontSize: '1.1rem', flexShrink: 0 }}>{loading ? '·' : done ? '✅' : '⭕'}</span>
      <span style={{ flex: 1, fontSize: '0.95rem', color: '#111827', fontWeight: done ? 400 : 500 }}>{label}</span>
      <span style={done ? doneBadge : pendingBadge}>{loading ? '' : done ? t('View') : t('Start')}</span>
    </button>
  )
}
...
function ActionRow({ icon, label, warn, onClick }) {
  return (
    <button onClick={onClick} style={{ ...row, borderBottom: 'none' }}>
      ...
      <span style={{ flex: 1, fontSize: '0.95rem', color: warn ? '#dc2626' : '#111827', fontWeight: 500 }}>{label}</span>
...
const sectionBox = { background: '#fff', borderRadius: 10, boxShadow: '0 1px 3px rgba(0,0,0,0.07)', overflow: 'hidden', border: '1px solid #f3f4f6' }
const row = { display: 'flex', alignItems: 'center', gap: '0.75rem', width: '100%', padding: '0.875rem 1rem', background: 'transparent', border: 'none', borderBottom: '1px solid #f3f4f6', cursor: 'pointer', textAlign: 'left' }
const doneBadge = { fontSize: '0.75rem', fontWeight: 600, background: '#f0fdf4', color: '#16a34a', border: '1px solid #bbf7d0', borderRadius: 6, padding: '0.2rem 0.6rem', flexShrink: 0 }
const pendingBadge = { fontSize: '0.75rem', fontWeight: 600, background: '#eff6ff', color: '#1d4ed8', border: '1px solid #bfdbfe', borderRadius: 6, padding: '0.2rem 0.6rem', flexShrink: 0 }
